...
class AppStore extends Component {
  ...
  filteredAppList = () => {
    const {inputValue} = this.state
    const filteredList = appsList.filter(eachItem =>
  ...
  render() {
    ...
    const filteredList = this.filteredAppList()
    return (
      <div className="bg-container">
        ...
        <ul className="app-list">
          {filteredList
            .filter(eachItem => activeTabId === eachItem.category)
            .map(eachItem => (
              <AppItem key={eachItem.appId} eachAppInfo={eachItem} />
            ))}
        </ul>
      </div>
    )
